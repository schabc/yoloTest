...
def compute_loss(predictions,targets,anchors,scales,num_classes=5,output_size=(13,13)):  #损失函数
...
def my_compute_loss(predictions,targets,anchors,scales,num_classes=5,output_size=(26,26)):
    scoor,sconf,snoob,sprob = scales #loss不同部分的前面系数
    ...
    # 【1】ground truth：期望值、真实值
    #targets = tf.reshape(targets,[-1,H,W,B,(5+C)])
    truths = tf.reshape(targets,[-1,H*W,B,(5+C)])
    _coords = truths[:, :, :, 0:4]  # ground truth [-1, H*W, B, 4]，网格xywh #真实坐标xywh  
    _probs = truths[:, :, :, 5:]  # class probability [-1, H*W, B, C] ，类别概率——one hot形式，C维
    _confs = truths[:, :, :, 4]  # 1 for object, 0 for background, [-1, H*W, B]，置信度，每个边界框一个
    # ground truth计算IOU-->_up_left, _down_right
    _wh = _coords[:, :, :, 2:4]#归一化后，真实高宽，
    _areas = _wh[:, :, :, 0] * _wh[:, :, :, 1]
    _centers = _coords[:, :, :, 0:2]#归一化后，真实中心点，与网格无关
    _up_left, _down_right = _centers - (_wh * 0.5), _centers + (_wh * 0.5)
    
    
    # 【2】decode the net prediction：预测值、网络输出值
    predictions = tf.reshape(predictions,[-1,H,W,B,(5+C)])  
    #这四个维度分别是batch_size, 特征图高度，特征图宽度，每个cell的框的数量，5+num_classes
    # t_x, t_y, t_w, t_h
    coords = tf.reshape(predictions[:,:,:,:,0:4],[-1,H*W,B,4]) # reshape成[batch_size, H*W, 5, 4]
    #下面两个都有点问题，不是全局归一化，真实值是全局归一化
    coords_xy = tf.nn.sigmoid(coords[:,:,:,0:2])  # 0-1，xy是相对于图像左上角的偏移量, 计算得出的是偏移量
    coords_wh = tf.exp(coords[:,:,:,2:4])*anchors#/np.reshape([W,H],[1,1,1,2]) # tf.sqrt()暂时不要，0-1，除以特征图的尺寸13，解码成相对于整张图片的wh
    coords = tf.concat([coords_xy,coords_wh],axis=3) # [batch_size, H*W, B, 4]
    # 置信度
    confs = tf.nn.sigmoid(predictions[:,:,:,:,4])    #经过一个sigmoid函数，使其落在[0-1]之间
    confs = tf.reshape(confs,[-1,H*W,B,1]) # 每个边界框一个置信度，每个cell有B个边界框，reshape成[batch_size, H*W, 5, 1]
    ...
    best_iou_mask = tf.equal(ious, tf.reduce_max(ious, axis=2, keepdims=True))   #每一个cell找到一个最大IOU，返回True或者False，在第三个维度上，第三个维度即是box数量=5
    best_iou_mask = tf.cast(best_iou_mask, tf.float32)   #将True或者False转换成1或者0， False->0, True->1
    mask = best_iou_mask * _confs  # [-1, H*W, B]  conf为真实值
    mask = tf.expand_dims(mask, -1)  # [-1, H*W, B, 1]   # 真实有目标的区域为1，没有的为0
    
    # 【4】计算各项损失所占的比例权重weight，好像维度不对
    confs_w = snoob * (1 - mask) + sconf * mask   #分别计算出没有目标的置信度和有目标的置信度， [-1,H*W,B,1]
    coords_w = scoor * mask   #得到有目标区域的分数
    probs_w = sprob * mask    #类别置信度
    weights = tf.concat([coords_w, confs_w, probs_w], axis=3)
